...
import { Package, ArrowLeft, Sparkles, CheckCircle2 } from 'lucide-react';
import bgHero from "../../assets/bgHero.png";

const NurseryInventory = () => {
    ...
    <div className="w-full min-h-screen bg-[#f7faf8] pt-16 sm:pt-20 font-sans">
        ...
        <div className="mx-auto max-w-7xl">
          ...
          <div className="max-w-3xl">
            <div className="mb-5 inline-flex items-center gap-2 rounded-full border border-emerald-300/25 bg-white/10 px-3 py-1.5 text-xs font-bold uppercase tracking-[0.14em] text-emerald-100">
              <Sparkles size={14} className="text-lime-300" /> Inventory Management
            </div>
            <h1 className="max-w-2xl text-3xl font-extrabold tracking-tight text-white sm:text-5xl">Nursery Inventory</h1>
            <p className="mt-5 max-w-2xl text-base leading-relaxed text-emerald-50/80 sm:text-lg">
              Manage your plant inventory, stock levels, and pricing — for nursery owners.
            </p>
          </div>
        </div>
      </section>
      ...
      <main className="relative z-10 mx-auto max-w-7xl px-4 pb-16 sm:px-6 sm:pb-20">
        <div className="-mt-7 rounded-3xl border border-slate-200 bg-white shadow-2xl shadow-emerald-950/10 p-8 sm:p-12 text-center">
          <div className="inline-flex h-20 w-20 items-center justify-center rounded-full bg-purple-100 mb-6">
            <Package className="h-10 w-10 text-purple-600" />
          </div>
          <h2 className="text-2xl font-extrabold text-slate-800 mb-3">Inventory Module</h2>
          <p className="text-slate-500 max-w-md mx-auto mb-6">
            This module allows nursery owners to add, edit, and manage plant inventory with real-time stock tracking and price updates.
          </p>
          <div className="flex flex-wrap justify-center gap-6 text-sm text-slate-600">
            <span className="flex items-center gap-2"><CheckCircle2 size={16} className="text-green-500" /> Add/Edit Plants</span>
            <span className="flex items-center gap-2"><CheckCircle2 size={16} className="text-green-500" /> Stock Management</span>
            <span className="flex items-center gap-2"><CheckCircle2 size={16} className="text-green-500" /> Price Updates</span>
          </div>
        </div>
      </main>
    </div>
