...
const ProductCard = ({ item, initialWishlistStatus, onWishlistToggle }) => {
  ...
  const handleWishlistToggle = async () => {
    ...
    const payload = { productId: item._id, variantId: item.variant._id };
    try {
      const options = {
      ...
      if (!response.ok) throw new Error(`Failed to ${action} product to wishlist`);
      setIsInWishlist((prev) => !prev);
      try { onWishlistToggle(item._id, item.variant._id, isInWishlist); } catch {}
    } catch (error) {
  ...
  const handleCart = async () => {
    if (!isLoggedIn) return;
    try {
      const response = await fetch(`${baseURL}api/cart/${userData}/add`, {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ productId: item._id, variantId: item.variant._id, quantity: 1 }),
      });
      const result = await response.json();
      if (response.ok) { navigate('/AgroShop/Cart'); }
      else { alert(result.message || "Failed to add item to cart."); }
    } catch (error) {
