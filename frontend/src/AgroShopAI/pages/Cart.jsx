...
const CartPage = () => {
    ...
    useEffect(() => {
        const fetchCartItems = async () => {
            if (isLoggedIn) {
                try {
                    const response = await api.get(`/api/cart/${userData}`);
    ...
    const updateQuantity = async (index, newQty) => {
        ...
        try {
            await api.put(`/api/cart/${userData}/update`, {
    ...
    const deleteItem = async (index) => {
        const item = cartItems[index];
        try {
            await api.delete(`/api/cart/${userData}/remove`, { data: { variantId: item.variantId._id } });
