"""API Router for AI-Based Fertilizer Identification & Smart Agricultural Advisory System.
Integrated into Krishi-AI Unified API.
"""
...
router = APIRouter(tags=["Fertilizer Advisor"])
...
def get_ai_client():
    api_key = os.getenv("OPENROUTER_API_KEY") or OPENROUTER_API_KEY or os.getenv("OPENAI_API_KEY") or OPENAI_API_KEY
    if not api_key:
        raise HTTPException(503, "Neither OPENROUTER_API_KEY nor OPENAI_API_KEY is configured on the server.")
    
    from openai import OpenAI
    if api_key.startswith("sk-or-v1"):
        return OpenAI(api_key=api_key, base_url="https://openrouter.ai/api/v1"), VISION_MODEL
    else:
...
def request_ai_completion(**kwargs):
    ...
    try:
        ...
        raise HTTPException(502, f"AI Gateway Error: {str(exc)}") from exc
...
@router.get("/api/fertilizer-advisor/health")
@router.get("/api/health/fertilizer-advisor")
def health() -> dict:
    return {
        "status": "ok",
...
@router.get("/api/fertilizer-advisor/catalog")
...
@router.post("/api/fertilizer-advisor/identify")
...
@router.post("/api/fertilizer-advisor/chat")
